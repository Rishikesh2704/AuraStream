...
export const Animeapi = createApi({
    ...
    baseQuery: fetchBaseQuery({ baseUrl: "https://anime-api-vert-alpha.vercel.app/api" }),
    ...
    endpoints: (builder) => ({
        ...
        Animeinfo: builder.query<InfoTypes,string>({
            ...
            transformResponse: (res:responseType<InfoTypes>):InfoTypes =>  res.results

        }),

        SearchSuggestions: builder.query<Partial<animeType>[],string>({
            query: (keyword) => `/search/suggest?keyword=${keyword }`,
            transformResponse: (res:responseType<Partial<animeType>[]>) => res.results
        ...
        Animesearch: builder.infiniteQuery<pagesType,string,number>({
            infiniteQueryOptions: {
                initialPageParam: 1,
                getNextPageParam: (last, allpage, lastPageParam:number) => lastPageParam+ 1
            },
            query({ queryArg, pageParam=1}) { return `/search?keyword=${queryArg}` },
            transformResponse: (res:responseType<pagesType>):pagesType => res.results
        }),
        ...
        GenreAnime: builder.infiniteQuery<pagesType,string,number>({
            ...
            query({queryArg,pageParam}) { return `/genre/${queryArg}?page=${pageParam}` },
            transformResponse: (res:responseType<pagesType>):pagesType => res.results
        })
