...
const TopAnimesSection = dynamic(() => import("./TopAnimesSection"), {
  ssr: false,
});
  ...
  return (
    <main tabIndex={0}>
      {Object.entries(ReduxHome).map(([key, value]) => {
        if (key === "spotlights")
          return (
            <SpotLightSection
              key={key}
              spotlightCoverAnimes={value as spotlightAnimeTypes[]}
            />
          );
      })}
      ...
      <section className="Home">
        {Object.entries(ReduxHome).map(([key, value]) => {
          if (key === "trending")
            return <TrendAnimes key={key + 1} animes={value as animeType[]} />;

          if (key === "topTen")
            return (
              <TopAnimesSection key={key + 2} animes={value as propsType} />
            );
